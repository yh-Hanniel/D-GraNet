        ...
        self.encoder = Encoder_ori(
            [
                LinearEncoder(
                    ...
                    num_dynamic_clusters=configs.num_dynamic_clusters,
                    num_static_clusters=configs.num_static_clusters,
                    m=configs.fcm_m,
                    threshold=configs.fcm_threshold,
                    # --- 下面是自动消融控制开关 ---
                    use_global_graph=getattr(configs, 'use_global_graph', True),
                    use_hyper_graph=getattr(configs, 'use_hyper_graph', True),
                    use_ffn=getattr(configs, 'use_ffn', True),
                    use_dynamic=getattr(configs, 'use_dynamic', True),
                    use_static_h=getattr(configs, 'use_static_h', True)
