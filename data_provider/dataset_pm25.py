...
class PM25_Dataset(Dataset):
    def __init__(self, root_path='./dataset/air', mode="train", eval_length=36, target_dim=36, validindex=0):
        ...
        path = os.path.join(root_path, "pm25_meanstd.pk")
        with open(path, "rb") as f:
            self.train_mean, self.train_std = pickle.load(f)
        if mode == "train":
            month_list = [1, 2, 4, 5, 7, 8, 10, 11]
            # 1st,4th,7th,10th months are excluded from histmask (since the months are used for creating missing
            # patterns in test dataset)
        ...
        df = pd.read_csv(
            os.path.join(root_path, 'ori_txt', "pm25_ground.txt"),
            index_col="datetime",
            parse_dates=True,
        )
        df_gt = pd.read_csv(
            os.path.join(root_path, 'ori_txt', "pm25_missing.txt"),
            index_col="datetime",
            parse_dates=True,
        )
        for i in range(len(month_list)):
            ...
            # print(f'current_df nan?: {np.any(np.isnan(current_df))}')
            # print(f'c_data nan?: {np.any(np.isnan(c_data))}')
    ...
    def __getitem__(self, org_index):
        ...
        return s["observed_data"], s["observed_mask"], s["gt_mask"], s["hist_mask"]
